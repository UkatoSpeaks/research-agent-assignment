...
def plan_node(state: AgentState) -> AgentState:
    plan = planner.create_plan(state["goal"])
...
def execute_node(state: AgentState) -> AgentState:
    ...
    # Deliberately fail exactly once.
    if state.get("force_failure", False) and not failure_injected:
        result = ToolResult(
            tool_name=step.tool,
            success=False,
            output=None,
            error="Deliberate test failure for recovery demonstration.",
...
def validate_node(state: AgentState) -> AgentState:
    ...
    return {
        **state,
        "errors": [
            *state.get("errors", []),
            result.error or "Tool validation failed.",
        ],
